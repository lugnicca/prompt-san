...
python-c "from promptsan import PromptSanitizer; print('PromptSan works!')"
```
...
###Run Examples
```bash
pythonexamples/example_basic_usage.py
pythonexamples/example_advanced_strategies.py
pythonexamples/example_complete.py
```
...
ForLLM-based anonymization, you need a local LLM server:
...
#Download a model
ollamapull dolphin-llama3:latest
...
###Basic Configuration
```python
frompromptsan import SanConfig
...
cdpromptsan
python-c "import sys; sys.path.insert(0, '.'); from promptsan import PromptSanitizer"
```

###LLM Connection Error
```bash
#Check if LLM server is running
curlhttp://localhost:1234/v1/models

#Or use a different port
config= SanConfig(llm_base_url="http://localhost:8080/v1")
```

###Permission Error (Windows)
```bash
#Run as administrator or use --user flag
pipinstall --user -r requirements.txt
```

## Verification

Runthis comprehensive test:
...
import json
withopen("examples/config_basic.json", "r") as f:
