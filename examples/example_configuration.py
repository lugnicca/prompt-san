...
def main():
    ...
    os.environ["PROMPTSAN_CUSTOM_DICT"] = (
        '{"Secret Project": "PROJECT", "Company A": "COMPANY"}'
    )
    ...
    sample_text = (
        "Dr. John Smith at Johns Hopkins (j.smith@jh.edu) treats patient #12345"
    )
    ...
    configurations = [
        {"name": "Basic Regex", "config": SanConfig(strategies=["regex"])},
        {
            "name": "Extended Regex",
            "config": SanConfig(
                strategies=["regex"],
        ...
        },
        {
            ...
            "config": SanConfig(
                strategies=["dict", "regex"],
                custom_dict={"Johns Hopkins": "HOSPITAL"},
                regex_patterns={
                    r"\b(?:Dr\.|Doctor)\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b": "DOCTOR",
                    r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b": "EMAIL",
                    r"\#\d+\b": "PATIENT_ID",
